...
export default function FilterData({ navigation, route }) {
  const { selectedValues, type } = route.params;
  ...
  const loadData = async () => {
    if (toload) {
      ...
      if (type == "both") genre = "non_veg,veg";
      else genre = type;

      axios
        .get(
          FIRETV_BASE_URL_STAGING +
            "/catalogs/recipes/items.gzip?item_language=eng&page_size=" +
            paginationLoadCount +
            "&page=" +
            pagenumber +
                  ...
                  onPress={() => {
                    {
                      item.medialistinlist
                        ...
                        : navigation.navigate("Shows", { seoUrl: item.seoUrl });
                    }
                  }}
                >
                  <FastImage
