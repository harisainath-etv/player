...
export default function FilterData({ navigation,route }) {
    ...
    const loadData = async () => {
        if (toload) {
            ...
            axios.get(FIRETV_BASE_URL_STAGING + "/catalogs/recipes/items.gzip?item_language=eng&page_size=" + paginationLoadCount + "&page=" + pagenumber + "&item_language=eng&region=" + region + "&auth_token=" + VIDEO_AUTH_TOKEN + "&access_token=" + ACCESS_TOKEN+"&genres="+selectedValues+"&sub_genres="+genre).then(response => {
