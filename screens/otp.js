...
export default function Otp({ navigation, route }) {
  ...
  const triggerfailureanalytics = async (
    name,
    error_type,
    method,
    device_id,
  ...
  const verifyOtp = async () => {
    ...
    if ((otp1 + otp2 + otp3 + otp4 + otp5 + otp6).length == 6) {
      if (otpkey == "loginMobile") {
        axios
          .post(
            FIRETV_BASE_URL_STAGING + "users/verify_otp",
            {
              ...
              user: {
                ...
                region: region,
                type: "msisdn",
                key: otp1 + otp2 + otp3 + otp4 + otp5 + otp6,
                user_id: loginMobile,
          ...
          .then((response) => {
            ...
            AsyncStorage.setItem(
              "region",
              response.data.data.profile_obj.region
            );
    ...
    } else Alert.alert("OTP Validation", "Please enter a valid otp.");
  };
      ...
      <View style={{ flex: 1, padding: 10, marginTop: 30 }}>
        <View style={styles.header}>
          ...
          <TouchableOpacity
            ...
            onPress={() => gobacknavigation(otpkey)}
          >
        ...
        <View style={{ justifyContent: "center", alignItems: "center" }}>
          <Text style={{ color: DETAILS_TEXT_COLOR }}>
            Enter the OTP sent to your mobile{" "}
          </Text>
          <Text style={{ color: DETAILS_TEXT_COLOR }}>number via SMS </Text>
          <Text
            style={{ color: NORMAL_TEXT_COLOR, fontSize: 18, marginTop: 20 }}
          >
            {LoginRegisterMobile}
          </Text>
        </View>
        <View style={{ marginTop: 50 }}>
          <Text style={{ color: DETAILS_TEXT_COLOR, fontSize: 12 }}>
            Enter 6 digit code
          </Text>
          <View style={{ flexDirection: "row", marginTop: 10 }}>
            ...
            <TextInput
              ...
              keyboardType="number-pad"
              onKeyPress={({ nativeEvent }) => {
                if (nativeEvent.key === "Backspace") otp1ref.current.focus();
                else otp3ref.current.focus();
              }}
            ></TextInput>
            <TextInput
              ref={otp3ref}
              ...
              keyboardType="number-pad"
              onKeyPress={({ nativeEvent }) => {
                if (nativeEvent.key === "Backspace") otp2ref.current.focus();
                else otp4ref.current.focus();
              }}
            ></TextInput>
            <TextInput
              ref={otp4ref}
              ...
              keyboardType="number-pad"
              onKeyPress={({ nativeEvent }) => {
                if (nativeEvent.key === "Backspace") otp3ref.current.focus();
                else otp5ref.current.focus();
              }}
            ></TextInput>
            <TextInput
              ref={otp5ref}
              ...
              keyboardType="number-pad"
              onKeyPress={({ nativeEvent }) => {
                if (nativeEvent.key === "Backspace") otp4ref.current.focus();
                else otp6ref.current.focus();
              }}
            ></TextInput>
            <TextInput
              ref={otp6ref}
              ...
              keyboardType="number-pad"
              onKeyPress={({ nativeEvent }) => {
                if (nativeEvent.key === "Backspace") otp5ref.current.focus();
              }}
            ></TextInput>
          </View>
