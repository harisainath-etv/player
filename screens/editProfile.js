...
export default function EditProfile({ navigation, route }) {
  ...
  const loadData = async () => {
    ...
    if (session != "" && session != null) {
      ...
      if (birthdate != "" && birthdate != null) setdob(birthdate);
      setgender(gender);
      if (address != "" && address != null) setaddress(address);
  ...
  const updateUser = async () => {
    ...
    await axios
      .put(FIRETV_BASE_URL_STAGING + "users/" + session + "/account", {
      ...
      <ScrollView style={{ marginTop: 20 }}>
        ...
        <View style={styles.inneroption}>
          ...
          {dob != "" && dob != null ? (
            ...
            >
              <Text
                style={{ color: NORMAL_TEXT_COLOR, fontSize: 16, padding: 10 }}
              >
                {dob}
              </Text>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  inneroption: {
    borderTopColor: DETAILS_TEXT_COLOR,
