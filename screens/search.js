        ...
        .then((response) => {
          ...
          if (response.data.data.items.length > 0) {
            for (var i = 0; i < response.data.data.items.length; i++) {
              if (
                response.data.data.items[i].hasOwnProperty("access_control")
              ) {
              ...
              if (response.data.data.items[i].media_list_in_list) {
                ...
                All.push({
                  uri: response.data.data.items[i].list_item_object
                    .banner_image,
                  theme: response.data.data.items[i].theme,
                  premium: premiumContent,
                  seoUrl: response.data.data.items[i].seo_url,
                  medialistinlist:
                    response.data.data.items[i].media_list_in_list,
                  friendlyId: friendlyId,
                  displayTitle: displayTitle,
                });
              } else {
                if (
                  response.data.data.items[i].thumbnails.hasOwnProperty(
                    "high_4_3"
                  ) ||
                  response.data.data.items[i].thumbnails.hasOwnProperty(
                    "high_3_4"
                  ...
                  All.push({
                    uri: response.data.data.items[i].thumbnails.high_4_3.url,
                    theme: response.data.data.items[i].theme,
                    premium: premiumContent,
                    seoUrl: response.data.data.items[i].seo_url,
                    medialistinlist:
            ...
            Final.push({
              friendlyId: response.data.data.friendly_id,
              data: All,
              layoutType: LAYOUT_TYPES[1],
              displayName: response.data.data.display_title,
            });
            All = [];
          } else {
                  ...
                  onPress={() => {
                    {
                      item.medialistinlist
                        ...
                        ? naviagtetopage("Episode", item.seoUrl, "", "Search")
                        : naviagtetopage("Shows", item.seoUrl, "", "Search");
                    }
                  }}
                >
                  <FastImage
      ...
      >
        ...
        >
          ...
          <View style={{ marginLeft: 0 }}>
            <TouchableOpacity onPress={loadDataCheck}>
              <Ionicons
                name="search-circle"
                size={30}
                color="#ffffff"
        ...
        {episodes.length == 0 && search == "" ? (
          ...
          >
            <Text style={{ color: DETAILS_TEXT_COLOR, fontSize: 22 }}>
              Search for Shows, Serials, Episodes, Movies, Recipes and Videos
            </Text>
          </View>
        ) : (
