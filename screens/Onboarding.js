...
const Onboarding = ({ navigation }) => {
    ...
    return (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Image source={require('../assets/images/slide0.jpg')} style={{ height: normalize(800), width: normalize(320), resizeMode: 'cover' }} />
            <TouchableOpacity onPress={async() => {
                setScreen(0);
            }} style={{ position: 'absolute', zIndex: 1000, bottom: normalize(80) }}>
                <Text style={{ fontWeight: 'bold', fontSize: 20, color: "red" }}>Skip</Text>
            </TouchableOpacity>
        </View>
    )
}
