...
export default function Profile({ navigation }) {
    ...
    Alert.alert("Sign Out", "Please confirm to sign out from this device.", [
      {
        ...
        onPress: async () => {
          ...
          axios
            .post(
              FIRETV_BASE_URL_STAGING + "users/" + session + "/sign_out",
              {
  ...
  const signoutall = async () => {
    Alert.alert(
      "Sign Out",
      "Please confirm to sign out from all the devices.",
      [
        {
          text: "Confirm",
          onPress: async () => {
            ...
            axios
              .post(
                FIRETV_BASE_URL_STAGING + "users/" + session + "/sign_out_all",
                {
                  auth_token: VIDEO_AUTH_TOKEN,
                  access_token: ACCESS_TOKEN,
                  type: "session",
              ...
              axios
                .get(
                  FIRETV_BASE_URL_STAGING +
                    "users/" +
                    sessionId +
                    "/false/delete_user_account?auth_token=" +
                    AUTH_TOKEN +
                    "&access_token=" +
                    ACCESS_TOKEN
                )
              ...
              axios
                .get(
                  FIRETV_BASE_URL_STAGING +
                    ...
                    "/" +
                    userinfoid +
                    "/delete_user_account?auth_token=" +
                    AUTH_TOKEN +
                    "&access_token=" +
                    ACCESS_TOKEN
                )
  ...
  return (
    <View style={{ backgroundColor: BACKGROUND_COLOR, flex: 1 }}>
        ...
        <TouchableOpacity
          onPress={() => {
            navigation.navigate("EditProfile", {
              letter:
                name != "" && name != null && name != "-" ? name.charAt(0) : "",
            });
          }}
          style={{}}
        >
      ...
      <ScrollView style={{ marginTop: 20 }}>
        ...
        <TouchableOpacity style={styles.inneroption} onPress={signout}>
          <Text style={styles.detailsvalue}>Sign Out</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.inneroption} onPress={signoutall}>
          <Text style={styles.detailsvalue}>Sign Out All Devices</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.inneroption} onPress={deleteMyAccount}>
...
const styles = StyleSheet.create({
  ...
  inneroption: {
    borderTopColor: FOOTER_DEFAULT_TEXT_COLOR,
