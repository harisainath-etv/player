...
export default function Confirmation({ navigation, route }) {
  ...
  const makepayment = async () => {
    ...
    var ext_account_email_id = await AsyncStorage.getItem(
    ...
    if (ext_account_email_id == "" || ext_account_email_id == null) {
      ...
      if (user_email_id == "" || user_email_id == null) var email = "";
      else var email = user_email_id;
    } else {
      var email = await AsyncStorage.getItem("ext_user_id");
    }
    if (payable_upgrade == "no") var palnupgrade = false;
    else var palnupgrade = true;
    var hashcalculated = stringMd5(
      SECRET_KEY + session + region + payable_plan_id
    );
    ...
    if (coupon != "")
      var paymentinfoobj = {
        coupon_code: coupon,
        net_amount: amount,
        price_charged: chargedamount.toString(),
        currency: payable_currency,
        packs: [
          {
            plan_type: "",
            category: payable_category,
            subscription_catalog_id: payable_catalog_id,
    ...
    if (region == "IN") {
      if (paymentgateway == "billdesk") {
        axios
          .post(
            FIRETV_BASE_URL_STAGING + "users/" + session + "/transactions",
            {
              ...
              transaction_info: {
                app_txn_id: 1,
                txn_message: payable_description,
                txn_status: "init",
                order_id: "",
                pg_transaction_id: "",
              },
      ...
      axios
        ...
        .then((resp) => {
          if (resp.data.data.code == "200") {
            ...
            navigation.navigate("HtmlWebview", {
              sessionid: resp.data.data.mpgs.session_id,
              description: resp.data.data.mpgs.description,
              transactionid: resp.data.data.transaction_id,
              referenceid: resp.data.data.mpgs.reference_id,
              name: resp.data.data.mpgs.name,
              line1: resp.data.data.mpgs.address1,
              line2: resp.data.data.mpgs.address1,
            });
          } else {
            alert(resp.data.data.message);
            navigation.dispatch(
  ...
  const applycoupon = async () => {
    ...
    axios
      .post(
        FIRETV_BASE_URL_STAGING + "users/" + session + "/apply_coupon_code",
        {
          access_token: ACCESS_TOKEN,
          auth_token: VIDEO_AUTH_TOKEN,
          category_pack_id: payable_category_id,
      ...
      .then((response) => {
        ...
        setdiscountmessage(
          "Promo Code Applied. You will get " +
            currency +
            " " +
            (amount - response.data.data.payment.net_amount) +
            " discount"
        );
      })
      .catch((error) => {
    ...
    <View style={{ backgroundColor: BACKGROUND_COLOR, flex: 1 }}>
      ...
      <View style={{ paddingLeft: 35, paddingRight: 35, marginTop: 20 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text
            style={{ color: SLIDER_PAGINATION_SELECTED_COLOR, fontSize: 15 }}
          >
            {planname} /{" "}
