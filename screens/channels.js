...
function Channels({ navigation, route }) {
    ...
    if (data.data.catalog_list_items.length > 0) {
      for (var i = 0; i < data.data.catalog_list_items.length; i++) {
        ...
        if (data.data.catalog_list_items[i].media_list_in_list) {
          ...
          All.push({
            uri: data.data.catalog_list_items[i].list_item_object.banner_image,
            theme: data.data.catalog_list_items[i].theme,
            premium: premiumContent,
            seoUrl: data.data.catalog_list_items[i].seo_url,
            medialistinlist: data.data.catalog_list_items[i].media_list_in_list,
  ...
  function changeTabData(pageFriendlyId) {
    if (pageFriendlyId != "live")
      navigation.navigate({
        name: "Home",
        params: { pageFriendlyId: pageFriendlyId },
        key: pageFriendlyId,
      });
    else
      navigation.navigate({
                ...
                renderItem={({ item, index }) => (
                  ...
                  >
                    <FastImage
                      resizeMode={FastImage.resizeMode.stretch}
                      key={index}
                      style={styles.imageSectionHorizontalSingle}
                      source={{
                        uri: item.uri,
                        priority: FastImage.priority.high,
                        cache: FastImage.cacheControl.immutable,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: BACKGROUND_COLOR }}>
      ...
      <Footer pageName="TV-CHANNELS"></Footer>
      <StatusBar
        animated
